...
Pleasing wave pattern that runs while nobody is interacting with the installation 
...
void update_servos() {
//Update servos with event in queue
...
bool if_special_time() {
	//check if time is special. If it is, return true
}

void time_pattern() {
	//runs pattern that happens for a special time

}

void default_pattern() {
	//loops through a single cycle of some default pattern, and adds to the queue
}
...
//We still need to adjust so th at it runs for the same length of time each loop. 
